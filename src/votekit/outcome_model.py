...
class Outcome(BaseModel):
    ...
    profile (a PreferenceProfile): a list of ballot types (schedule preferences) and their frequency of times
    ...
    def get_all_eliminated(self) -> list[str]:
        """returns all winners from all rounds so far in order of last eliminated to first eliminated
        """
    ...
    def get_round_outcome(self,roundNum: int) -> dict: #{'elected':list[str], 'eliminated':list[str]}
  ...
  ###############################################################################################
    ...
    def difference_remaining_candidates(self, prevOutcome1: PreferenceProfile, prevOutcome2: PreferenceProfile) -> float:
        """returns the fractional difference in number of remaining candidates; assumes ballots don't change by round
        """
