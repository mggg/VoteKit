...
class Borda:
    ...
    def run_borda_step(self):
        ...
        candidates = self.profile.get_candidates()
        borda_scores = {}   # {candidate : int borda_score}
        candidate_rank_freq = {}   # {candidate : [1st rank total, 2nd rank total,..., n rank total]}
        ...
        for ballot in self.profile.get_ballots():
            ...
            for candidate in ballot.ranking:
                ...
                if candidate not in candidate_rank_freq:
                    candidate_rank_freq[candidate] = [0 for _ in range(len(ballot.ranking))]
        ...
        for key in candidate_rank_freq:
            borda_scores[key] = sum([x * y for x, y in zip(candidate_rank_freq[key], self.borda_weights)])
        ...
        return PreferenceProfile(ballots=self.profile.get_ballots()), Outcome(remaining = set(), 
                                                                         elected = set(winners), 
