...
def add_null_keys(data: DataMapping) -> dict[str, dict[CategoryLabel, float]]:
    ...
    Args:
        data (dict[str, dict[str | int, float]]): Categorical data to be cleaned.
            The value of each dict should be the frequency of the key which is the category name.

    Returns:
        dict[str, dict[str | int, float]]: Cleaned data.
    """

    x_labels: list[CategoryLabel] = []
    _data = cast(Mapping[str, Mapping[CategoryLabel, float]], data)
    clean_data = {label: dict(data_dict) for label, data_dict in _data.items()}
...
def _set_default_bar_plot_args(
    *,
    ...
    category_ordering: Optional[CategoryLabelList],
...
def _validate_bar_plot_args(
    *,
    data: DataMapping,
    category_ordering: CategoryLabelList,
    categories_legend: Optional[CategoryLabelMapping],
    bar_width: float,
    ...
    Args:
        data (dict[str, dict[str | int, float]] | dict[str, dict[str, float]]
            | dict[str, dict[int, float]]): Categorical data to be plotted.
            Top level keys are data set labels.
            Inner keys are categories, and inner values are the height of the bars.
        category_ordering (list[str | int] | list[str] | list[int]): Ordering of x labels.
            Must match data keys.
        categories_legend (dict[str | int, str] | dict[str, str] | dict[int, str] | None):
            Dictionary mapping data categories to relabeling. If provided, generates a second legend
            for data categories and relabels the x-axis accordingly.
            Can be a subset of the data keys.
...
def _normalize_data_dict(
    data_dict: Mapping[str | int, float] | Mapping[str, float] | Mapping[int, float],
) -> dict[str | int, float] | dict[str, float] | dict[int, float]:
    """
    Normalizes data so number of total observations is 1. Raises a ValueError if the total
    mass is 0.

    Args:
        data (dict[str | int, float] | dict[str, float] | dict[int, float]):
            Single data dictionary whose keys are categories and values are bar heights.

    Returns:
        dict[str | int, float] | dict[str, float] | dict[int, float]: Normalized data.
...
def _prepare_data_bar_plot(
    *,
    ...
    """
    plot_data: Mapping[str, Mapping[str | int, float]] = cast(
        Mapping[str, Mapping[str | int, float]], data
    )
    if normalize:
        plot_data = cast(
    ...
    _category_ordering = cast(list[CategoryLabel], category_ordering)
    y_data = [
        [data_dict[x_label] for x_label in _category_ordering] for data_dict in plot_data.values()
    ]
...
def _plot_datasets_on_bar_plot(
    *,
    ...
    Args:
        ...
        categories_legend (dict[str | int, str] | dict[str, str] | dict[int, str] | None):
            Dictionary mapping data categories to relabeling.
            If provided, generates a second legend for data categories
            and relabels the x-axis accordingly. Can be a subset of the data keys.
    ...
    """
    if categories_legend:
        _category_ordering = cast(list[str | int], category_ordering)
        _categories_legend = cast(Mapping[str | int, str | int], categories_legend)
        tick_labels = [_categories_legend.get(c, c) for c in _category_ordering]
    else:
...
def _add_data_sets_legend_bar_plot(
    *,
    ...
    Args:
        ...
        data_set_to_color (dict): Dictionary mapping data set labels to colors.
        categories_legend (dict[str | int, str] | dict[str, str] | dict[int, str] | None):
            Dictionary mapping x-axis categories to description in legend.
            Defaults to None, in which case legend is just x-axis labels.
...
def _add_categories_legend_bar_plot(
    *,
    ax: Axes,
    categories_legend: CategoryLabelMapping,
    legend_font_size: float,
    legend_loc: str,
    legend_bbox_to_anchor: Tuple[float, float],
    ...
    Args:
        ax (Axes): Matplotlib axes containing barplot.
        categories_legend (dict[str | int, str] | dict[str, str] | dict[int, str]):
            Dictionary mapping x-axis categories to description in legend.
        legend_font_size (float): The font size to use for the legend.
        legend_loc(str): The location parameter to pass to ``Axes.legend(loc=)``.
        legend_bbox_to_anchor (Tuple[float, float]): The bounding box to anchor the legend to.
...
def multi_bar_plot(
    data: Mapping[str, Mapping[CategoryLabel, float]]
    ...
    *,
    ...
    category_ordering: Optional[CategoryLabelList] = None,
    x_axis_name: Optional[str] = None,
    y_axis_name: Optional[str] = None,
    title: Optional[str] = None,
    show_data_set_legend: bool = False,
    categories_legend: Optional[CategoryLabelMapping] = None,
    ...
    Args:
        ...
        categories_legend (dict[str | int, str] | dict[str, str] | dict[int, str] |  None):
            Dictionary mapping data categories to relabeling.
            Defaults to None. If provided, generates a second legend for data
...
def bar_plot(
    data: Mapping[str | int, float] | Mapping[str, float] | Mapping[int, float],
    *,
    ...
    Args:
        data (dict[str | int, float] | dict[str, float] | dict[int, float]):
            Categorical data set to be plotted.
            Keys are categories, and values are the height of the bars.
    ...
    return multi_bar_plot(
        cast(DataMapping, {data_set_label: data}),
        normalize=normalize,
