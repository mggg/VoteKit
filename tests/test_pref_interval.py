...
def test_from_dirichlet():
    pi = PreferenceInterval.from_dirichlet(candidates=["A", "B", "C"],
                                  alpha = .01)
    
    assert isinstance(pi, PreferenceInterval)

def test_from_interval():
    pi = PreferenceInterval(interval = {"A": .2, "B": .3, "C": .5})
    assert isinstance(pi, PreferenceInterval)
...
def test_normalize():
    ...
    with pytest.raises(ZeroDivisionError, match = "There are no candidates with non-zero support."):
