...
def test_id_and_weight_error():
    with pytest.raises(
        ...
        load_csv(CSV_DIR / "valid_cvr.csv", rank_cols=[0, 1, 2], weight_col=3, id_col=4)
...
def test_columns_out_of_range_error():
    ...
    with pytest.raises(
        ...
        load_csv(CSV_DIR / "valid_cvr.csv", rank_cols=[0, 1, 2], id_col=5)
    ...
    with pytest.raises(
        ...
        load_csv(CSV_DIR / "valid_cvr.csv", rank_cols=[0, 1, 2], weight_col=5)
...
def test_header_error():
    with pytest.raises(ValueError, match="Header -1 must be non-negative."):
        load_csv(CSV_DIR / "valid_cvr.csv", rank_cols=[0, 1, 2], header=-1)
...
def test_load_csv_w_weight():
    profile = load_csv(CSV_DIR / "valid_cvr.csv", rank_cols=[0, 1, 2], weight_col=3)
...
def test_load_csv_w_id():
    profile = load_csv(CSV_DIR / "valid_cvr.csv", rank_cols=[0, 1, 2], id_col=4)
