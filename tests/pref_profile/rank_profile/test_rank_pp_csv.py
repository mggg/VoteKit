...
def test_csv_misformatted_header_values_error():
    ...
    with pytest.raises(ValueError, match="If not using v2 format, Row 2 should contain tuples"):
        RankProfile.from_csv(f"{filepath}/test_csv_pp_misformat_header_value_2_invalid_tuples.csv")
...
def test_csv_misformatted_ballot_ranking_error():
    with pytest.raises(ValueError, match=("Ballot in row 11 has improperly formatted ranking ")):
        RankProfile.from_csv(f"{filepath}/test_csv_pp_misformat_ballot_ranking_brace.csv")

    with pytest.raises(ValueError, match=("Ballot in row 11 has undefined candidate prefix")):
        RankProfile.from_csv(f"{filepath}/test_csv_pp_misformat_ballot_ranking_prefix.csv")
