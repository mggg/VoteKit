...
profile_tie = PreferenceProfile(
    ballots=[Ballot(scores={"A1": 1}, weight=10), Ballot(scores={"B1": 1}, weight=10)]
)
...
def test_init():
    e = OpenListPR(
        profile_simple, party_to_candidate_map=party_map_simple, m=3, tiebreak="random"
    )
...
def test_ties():
    e_random = OpenListPR(
        profile_tie, party_to_candidate_map=party_map_tie, m=1, tiebreak="random"
    )
...
def test_state_list():
    e = OpenListPR(
        profile_simple, party_to_candidate_map=party_map_simple, m=2, tiebreak="random"
    )
...
def test_get_eliminated():
    e = OpenListPR(
        profile_simple, party_to_candidate_map=party_map_simple, tiebreak="random", m=2
    )
...
def test_get_status_df():
    ...
    e = OpenListPR(
        profile_no_ties, party_to_candidate_map=party_map, tiebreak="random", m=2
    )
...
def test_errors():
    ...
    with pytest.raises(ValueError, match="m must be positive"):
        OpenListPR(
            profile_simple,
            m=0,
            party_to_candidate_map=party_map_simple,
            tiebreak="random",
        )
...
def test_validate_profile():
    ...
    bad1 = PreferenceProfile(ballots=[Ballot(scores={"X": 2, "Y": 1}, weight=10)])
...
# Built ballots
ballots = [
    Ballot(scores={random.choice(candidates): 1}, weight=1) for _ in range(n_ballots)
]
...
def test_stress():
    ...
    for _ in range(10):
        ...
        _ = OpenListPR(
            profile_stress,
            m=10,
            party_to_candidate_map={f"Party{c}": [c] for c in candidates},
            tiebreak="random",
        )
        time1 = time.time()
