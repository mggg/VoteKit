...
def test_init():
    e = BlocPlurality(profile_no_tied_bloc_plurality, n_seats=2)
    assert e.get_elected() == (frozenset({"A"}), frozenset({"B"}))


def test_ties():
    e_random = BlocPlurality(profile_tied_bloc_plurality, n_seats=1, tiebreak="random")
...
def test_validate_profile():
    ...
    with pytest.raises(TypeError, match="must be of type ScoreBallot"):
        ...
        BlocPlurality(cast(ScoreProfile, profile), n_seats=1)
