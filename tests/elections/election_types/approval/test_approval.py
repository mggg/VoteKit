...
def test_init():
    e = Approval(profile_no_tied_approval, n_seats=2)
    assert e.get_elected() == (frozenset({"A", "B"}),)


def test_ties():
    e_random = Approval(profile_tied_approval, n_seats=1, tiebreak="random")
...
def test_get_status_df():
    ...
    e = Approval(profile_no_ties, n_seats=2)
