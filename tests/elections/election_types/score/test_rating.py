...
def test_init():
    e = Rating(profile_no_tied_rating, per_candidate_limit=2)
    assert e.get_elected() == (frozenset({"A"}),)

    e = Rating(profile_no_tied_rating, n_seats=2, per_candidate_limit=2)
    assert e.get_elected() == (frozenset({"A"}), frozenset({"C"}))


def test_ties():
    e_random = Rating(profile_tied_rating, n_seats=1, tiebreak="random", per_candidate_limit=3)
    assert len([c for s in e_random.get_elected() for c in s]) == 1

    e_random = Rating(profile_tied_rating, n_seats=2, tiebreak="random", per_candidate_limit=3)
    assert len([c for s in e_random.get_elected() for c in s]) == 2

    e_random = Rating(profile_tied_rating, n_seats=3, tiebreak="random", per_candidate_limit=3)
...
def test_validate_profile():
    ...
    with pytest.raises(ValueError, match="Not enough candidates received votes to be elected."):
        Rating(ScoreProfile(candidates=["A"]), n_seats=1)
