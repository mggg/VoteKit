...
def test_init():
    e = RankedPairs(electowiki_profile, n_seats=1)
    assert e.get_elected() == (frozenset({"Nashville"}),)


def test_init_with_empty():
    e = RankedPairs(profile_with_skips, n_seats=3)
    assert e.get_elected() == convert_to_fs_tuple(["D", "A", "C"])


def test_init_with_empty_errors_out_when_too_many_elected():
    with pytest.raises(ValueError):
        RankedPairs(profile_with_skips, n_seats=4)


def test_limit_case():
    e = RankedPairs(test_profile_limit_case, n_seats=2)
    assert e.get_elected() == convert_to_fs_tuple(["C", "A"])


def test_borda_ambiguous_profile_returns_lexicographic_order():
    e = RankedPairs(borda_ambiguous_profile, n_seats=1)
    assert e.get_elected() == convert_to_fs_tuple(["A"])
    e = RankedPairs(borda_ambiguous_profile, n_seats=2)
    assert e.get_elected() == convert_to_fs_tuple(["A", "B"])
    e = RankedPairs(borda_ambiguous_profile, n_seats=3)
    assert e.get_elected() == convert_to_fs_tuple(["A", "B", "C"])


def test_dominating_ambigous_profile_returns_lexicographic_order():
    e = RankedPairs(dominating_ambiguous_profile, n_seats=1)
    assert e.get_elected() == convert_to_fs_tuple(["A"])
    e = RankedPairs(dominating_ambiguous_profile, n_seats=2)
    assert e.get_elected() == convert_to_fs_tuple(["A", "B"])
    e = RankedPairs(dominating_ambiguous_profile, n_seats=3)
    assert e.get_elected() == convert_to_fs_tuple(["A", "B", "C"])


def test_tied_set():
    e = RankedPairs(profile_tied_set, n_seats=1)
    assert e.get_elected() == convert_to_fs_tuple(["A"])
    e = RankedPairs(profile_tied_set, n_seats=2)
    assert e.get_elected() == convert_to_fs_tuple(["A", "B"])
    e = RankedPairs(profile_tied_set, n_seats=3)
    assert e.get_elected() == convert_to_fs_tuple(["A", "B", "C"])


def test_profile_cycle():
    e = RankedPairs(profile_cycle, n_seats=1)
    assert e.get_elected() == convert_to_fs_tuple(["A"])
    e = RankedPairs(profile_cycle, n_seats=2)
    assert e.get_elected() == convert_to_fs_tuple(["A", "B"])
    e = RankedPairs(profile_cycle, n_seats=3)
    assert e.get_elected() == convert_to_fs_tuple(["A", "B", "C"])


def test_tied_borda():
    e = RankedPairs(profile_tied_borda, n_seats=1)
    assert e.get_elected() == convert_to_fs_tuple(["A"])
    e = RankedPairs(profile_tied_borda, n_seats=2)
    assert e.get_elected() == convert_to_fs_tuple(["A", "B"])
    e = RankedPairs(profile_tied_borda, n_seats=3)
    assert e.get_elected() == convert_to_fs_tuple(["A", "B", "C"])
...
def test_errors():
    ...
    with pytest.raises(ValueError, match="Not enough candidates received votes to be elected."):
        RankedPairs(profile_tied_set, n_seats=4)
...
def test_large_set_timing():
    ...
    assert end - start < 120, (
        f"RankedPairs runtime took too long. Expected < 120 seconds, got {end - start} seconds."
    )
