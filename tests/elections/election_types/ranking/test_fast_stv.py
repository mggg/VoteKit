...
def test_stv_cands_cast():
    ...
    assert STV(profile, simultaneous=False, n_seats=3).get_elected() == ({"C"}, {"A"}, {"B"})
...
def test_random_transfers():
    ...
    assert STV(reducto_ad_absurdum, n_seats=2, transfer="cambridge_random").get_elected() == (
        frozenset({"A"}),
        frozenset({"B"}),
    )
    assert STV(reducto_ad_absurdum, n_seats=2, transfer="fractional_random").get_elected() == (
        frozenset({"A"}),
        frozenset({"C"}),
    )
    assert STV(reducto_ad_absurdum, n_seats=2, transfer="fractional").get_elected() == (
        frozenset({"A"}),
        frozenset({"C"}),
    )
...
def test_simult_not_same_as_1b1():
    ...
    assert STV(profile, n_seats=3, simultaneous=True).get_elected() == (  # TODO: change this test
        frozenset({"A", "B"}),
        frozenset({"D"}),
    )
    assert STV(profile, n_seats=3, simultaneous=False).get_elected() == (
        frozenset({"A"}),
        frozenset({"B"}),
        frozenset({"C"}),
    )
