...
class TestElection(Election):
    ...
    def __init__(self, profile: RankProfile, sort_high_low: bool = True):
        def score(profile: RankProfile) -> dict:
    ...
    def _validate_profile(self, profile):
        for ballot in profile.ballots:
            if not ballot.ranking:
                raise TypeError("RankBallots must having rankings.")
    ...
    def _run_step(
        ...
        new_profile = RankProfile(ballots=profile.ballots[1:])
...
profile = RankProfile(
    ballots=[
        ...
        RankBallot(ranking=[{"C"}, {"D"}]),
        RankBallot(ranking=[{"E"}, {"F"}]),
    ]
)
profile_1 = RankProfile(
    ballots=[RankBallot(ranking=[{"C"}, {"D"}]), RankBallot(ranking=[{"E"}, {"F"}])]
)
profile_2 = RankProfile(ballots=[RankBallot(ranking=[{"E"}, {"F"}])])
